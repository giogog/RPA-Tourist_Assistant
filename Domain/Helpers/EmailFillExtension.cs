    ...
    public static List<Client> FillClientModel(this Dictionary<string,List<string>> context)
    {
        try{
            ...
            for(int i = 0; i<memberNum;i++)
            {
                ...
                clientContext.DepartureAirport = context.FirstOrDefault(v => v.Key.Contains("Airport of departure to Colombia") || v.Key.Contains("Aeropuerto de salida a Colombia") || v.Key.Contains("Aeropuerto de salida de Colombia")).ReturnDictionaryStringValue(0);
                ...
                clientContext.ArrivalDate = context.FirstOrDefault(v => v.Key.Contains("Date of arrival in Colombia") || v.Key.Contains("Fecha de salida de Colombia") || v.Key.Contains("Fecha de llegada a Colombia")).ReturnDictionaryStringValue(0);
                clientContext.FlightNum = context.FirstOrDefault(v => v.Key.Contains("Flight number (you will be landing under in Colombia)") || v.Key.Contains("Número de vuelo") || v.Key.Contains("Número de vuelo (bajo el cual aterrizará en Colombia)")).ReturnDictionaryStringValue(0);            
